...
Видалено всі console.log () (окрім системних для моніторингу ) та замінено != на !== у всьому проєкті для кращої якості коду.
...
Для коректної роботи системи прав доступу та функціоналу демо-режиму, необхідно виконати наступні кроки після першого запуску:

**1. Перший користувач та права**


У системі реалізована логіка автоматичного призначення ролей. **Перший зареєстрований користувач у базі даних** автоматично отримує роль `admin`. Усі наступні користувачі за замовчуванням отримують роль `user`.

**2. Налаштування режиму "гість" (Demo User)**


Для роботи кнопки **"Увійти як гість"** (Demo User) необхідно синхронізувати ID з базою даних:


* Зареєструйте користувача з поштою `test@test.com` через інтерфейс реєстрації.

* Знайдіть створеного користувача у вашій базі даних (MongoDB у випадку цього проєкту).

* Скопіюйте його `_id` (наприклад, 64b...).

* Відкрийте файл `Jobify/utils/constants.js` та оновіть значення `TEST_USER_ID`:


```bash
// Jobify/utils/constants.js

// Вставте сюди ID користувача test@test.com з вашої БД
export const TEST_USER_ID = "ваш_ID_тут"; 

export const DEMO_USER = {
  email: "test@test.com",
  password: "secret123", // або інший
};
```
